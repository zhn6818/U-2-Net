...
def parse_args():
    ...
    parser.add_argument('--class_names', type=str, default=None, help='类别名称,逗号分隔的字符串,如"BG,A,B,C"')
...
def get_default_class_names(num_classes):
    ...
    for i in range(1, num_classes):
        default_names.append(f'Class_{i}')
    return default_names

def get_color_map(class_names):
    """获取类别对应的颜色映射"""
    # 预定义的颜色 (如果类别较多，将循环使用这些颜色)
    predefined_colors = [
        [0, 0, 0],       # 背景为黑色
        [255, 0, 0],     # 红色
        [255, 255, 0],   # 黄色
    ...
    for i, class_name in enumerate(class_names):
        # 为每个类别分配颜色，如果类别数超过预定义颜色数，将会循环使用
        color_map[class_name] = predefined_colors[i % len(predefined_colors)]
    
    return color_map

def get_class_name_by_index(idx, class_names):
    """根据索引获取类别名称"""
...
def create_colored_pred_mask(pred, class_names, threshold=0.5):
    """从预测概率创建彩色掩码图像"""
    # 获取颜色映射
    color_map = get_color_map(class_names)
...
def visualize_and_save(image, mask, output_path, class_names, alpha=0.7, pred=None):
    """可视化图像和掩码，并保存结果"""
    # 创建彩色掩码
    ...
    if pred is not None:
        ...
        # 创建彩色预测掩码
        colored_pred = create_colored_pred_mask(pred, class_names)
    ...
    for i, (class_name, color) in enumerate(color_map.items()):
        if class_name == 'BG' and i == 0:  # 背景可以选择不在图例中显示
            continue
        # 转换颜色为0-1范围
...
def process_images(args):
    ...
    if args.class_names:
        ...
        # 确保类别数量与指定的num_classes一致
        if len(class_names) != args.num_classes:
            print(f"Warning: Number of provided class names ({len(class_names)}) does not match num_classes ({args.num_classes})")
            print(f"Using default class names for {args.num_classes} classes")
            class_names = get_default_class_names(args.num_classes)
    else:
        # 使用默认类别名称
        class_names = get_default_class_names(args.num_classes)

    print(f"Using {args.num_classes} classes: {class_names}")
    ...
    for image_path in image_files:
        ...
        # 可视化并保存
        visualize_and_save(image, mask, output_path, class_names, args.alpha, pred)
