...
## 最新改进
1. 增加了边缘感知损失（boundary loss），提高模型对图像边缘区域的处理能力
2. 优化了数据增强策略，增加了模糊和锐化处理
3. 添加了学习率调度器，动态调整学习率
4. 增加了验证集评估，更好地监控模型训练效果
5. 边缘感知损失现在可以配置为可选项，可以通过配置开启或关闭

## 使用说明
### 如何禁用边界损失（Boundary Loss）
在训练时，如果不想使用边界损失，可以通过以下方式关闭：

1. 修改`Config`类中的参数：
```python
config = Config()
config.use_boundary_loss = False  # 关闭边界损失
```
...
class Config:
    def __init__(self):
        # ... 其他配置 ...
        self.use_boundary_loss = False  # 默认关闭边界损失
        self.boundary_weight = 0.5  # 边界损失的权重（当启用边界损失时）
```

边界损失的作用是增强模型对目标边缘的感知能力，对于需要精细边缘的分割任务非常有用。但在某些情况下（如分割目标边界不明显或不重要的场景），可能会导致过度关注边缘而忽略全局特征，此时可以考虑关闭边界损失。
