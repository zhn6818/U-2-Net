...
from data_loader import ColorJitter, SalObjDataset, RescaleT, ToTensorLab
import random
import torch
...
def test_dataloader_with_color_jitter():
    ...
    plt.tight_layout()
    output_path = os.path.join(figures_dir, "dataloader_color_jitter_demo.png")
    plt.savefig(output_path)
    plt.close()
    
    print(f"数据加载器测试完成! 结果已保存为 {output_path}")
...
def main():
    ...
    print("\n=== 测试2: 使用数据加载器测试颜色抖动 ===")
    test_dataloader_with_color_jitter()
